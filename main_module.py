...
#
            ...
            try:SetProcessDefaultLayout = ctypes.windll.kernel32.SetProcessDefaultLayout
            except:print('EC/12/SetProcessDefaultLayout')
            try:IsProcessInJob = ctypes.windll.kernel32.IsProcessInJob;IsProcessInJob(pid2, jb_id)
            except:print('EC/13/RemoveProcessFromJob');F=1
            NtSetInformationProcess = ctypes.windll.ntdll.NtSetInformationProcess;NtSetInformationProcess(pid2, 22, 0, 0)
#
            import ctypes
            import struct
            ...
            def patch_memory(address, value):
                ...
                PAGE_READWRITE = 0x04
                h_process = ctypes.windll.kernel32.OpenProcess(PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE, 0, int(address.split("-")[0]))
                if h_process:
                    old_protection = ctypes.c_long(0)
                    ctypes.windll.kernel32.VirtualProtectEx(h_process, int(address, 16), len(value), PAGE_READWRITE, ctypes.byref(old_protection))
                    ctypes.windll.kernel32.WriteProcessMemory(h_process, int(address, 16), value, len(value), None)
                    ctypes.windll.kernel32.VirtualProtectEx(h_process, int(address, 16), len(value), old_protection, ctypes.byref(old_protection))
            ...
            def hide():
                ...
                myNtQueryInformationProcessRVA = ctypes.windll.kernel32.GetProcAddress(ctypes.windll.kernel32.GetModuleHandleA(b"ntdll.dll"), b"NtQuerySystemInformation")
                for proc_name in process_mgr:
                    ...
                    if proc_id:
                        ...
                        if h_proc and h_proc != -1:
                            ...
                            NtQueryInformationProcessRVA = (myNtQueryInformationProcessRVA - get_module_base_address(ctypes.windll.kernel32.GetCurrentProcessId(), "ntdll.dll"))
...
def ACC(RL):
    ...
    if DS == 1:
        ...
        for pll in range(1,TLP):pyautogui.move(X,Y,T)
        sl(st)
        if RL == 'R':
...
#
        ...
        while True:
            ...
            if keymap == 1:
                ...
                print('Minecraft')
                leftCL = 'r';rightCL = 'f'
                tomkey ('w','a','s','d','r','space','ctrl','shift',on_start_pressed);synck=['w','a','s','d','r','space']
                ...
                def godkey(ckeymap):
                    ...
                    try:
                        ...
                        elif ckeymapN == 2:leftCL = ckeymap[0];rightCL = ckeymap[1];tomkey (ckeymap[0],'F6','F7','F8','F9','F10','F11','F12',on_start_pressed);tomkey (ckeymap[1],'F6','F7','F8','F9','F10','F12','F11',on_start2_pressed);keyboard.add_hotkey(ckeymap[0], on_start_pressed);keyboard.add_hotkey(ckeymap[1], on_start2_pressed);synck=[]
                        elif ckeymapN == 3:leftCL = ckeymap[0];rightCL = ckeymap[1];tomkey (ckeymap[0],ckeymap[2],'F7','F8','F9','F10','F11','F12',on_start_pressed);tomkey (ckeymap[1],ckeymap[2],'F7','F8','F9','F10','F12','F11',on_start2_pressed);keyboard.add_hotkey(ckeymap[0], on_start_pressed);keyboard.add_hotkey(ckeymap[1], on_start2_pressed);synck=[ckeymap[2]]
                        elif ckeymapN == 4:leftCL = ckeymap[0];rightCL = ckeymap[1];tomkey (ckeymap[0],ckeymap[2],ckeymap[3],'F8','F9','F10','F11','F12',on_start_pressed);tomkey (ckeymap[1],ckeymap[2],ckeymap[3],'F8','F9','F10','F12','F11',on_start2_pressed);keyboard.add_hotkey(ckeymap[0], on_start_pressed);keyboard.add_hotkey(ckeymap[1], on_start2_pressed);synck=[ckeymap[2],ckeymap[3]]
                        elif ckeymapN == 5:leftCL = ckeymap[0];rightCL = ckeymap[1];tomkey (ckeymap[0],ckeymap[2],ckeymap[3],ckeymap[4],'F9','F10','F11','F12',on_start_pressed);tomkey (ckeymap[1],ckeymap[2],ckeymap[3],ckeymap[4],'F9','F10','F12','F11',on_start2_pressed);keyboard.add_hotkey(ckeymap[0], on_start_pressed);keyboard.add_hotkey(ckeymap[1], on_start2_pressed);synck=[ckeymap[2],ckeymap[3],ckeymap[4]]
                        elif ckeymapN == 6:leftCL = ckeymap[0];rightCL = ckeymap[1];tomkey (ckeymap[0],ckeymap[2],ckeymap[3],ckeymap[4],ckeymap[5],'F10','F11','F12',on_start_pressed);tomkey (ckeymap[1],ckeymap[2],ckeymap[3],ckeymap[4],ckeymap[5],'F10','F12','F11',on_start2_pressed);keyboard.add_hotkey(ckeymap[0], on_start_pressed);keyboard.add_hotkey(ckeymap[1], on_start2_pressed);synck=[ckeymap[2],ckeymap[3],ckeymap[4],ckeymap[5]]
                        elif ckeymapN == 7:leftCL = ckeymap[0];rightCL = ckeymap[1];tomkey (ckeymap[0],ckeymap[2],ckeymap[3],ckeymap[4],ckeymap[5],ckeymap[6],'F11','F12',on_start_pressed);tomkey (ckeymap[1],ckeymap[2],ckeymap[3],ckeymap[4],ckeymap[5],ckeymap[6],'F12','F11',on_start2_pressed);keyboard.add_hotkey(ckeymap[0], on_start_pressed);keyboard.add_hotkey(ckeymap[1], on_start2_pressed);synck=[ckeymap[2],ckeymap[3],ckeymap[4],ckeymap[5],ckeymap[6]]
                        elif ckeymapN == 8:leftCL = ckeymap[0];rightCL = ckeymap[1];tomkey (ckeymap[0],ckeymap[2],ckeymap[3],ckeymap[4],ckeymap[5],ckeymap[6],ckeymap[7],'F11',on_start_pressed);tomkey (ckeymap[1],ckeymap[2],ckeymap[3],ckeymap[4],ckeymap[5],ckeymap[6],ckeymap[7],'F11',on_start2_pressed);keyboard.add_hotkey(ckeymap[0], on_start_pressed);keyboard.add_hotkey(ckeymap[1], on_start2_pressed);synck=[ckeymap[2],ckeymap[3],ckeymap[4],ckeymap[5],ckeymap[6],ckeymap[7]]
        ...
        if fini == 12:
            ...
            def KeyRun(KeyB):
                ...
                keyboard.press(KeyB)
                RPL=ri(2,3)
            def rts():rf1=ri(0,1);rf2=ri(1,999);slt=rf1+(rf2*(10**-3));sl(slt);print(slt)
            def AutoKeyBS():
                global Chlps
                global ChlpsX
                global tryx
                global synck
                if tryx==True:
                    try:a=Chlps[0];d=Chlps[1];c=Chlps[2];f=Chlps[3];g=Chlps[4];print('i/77');tryx=False;ChlpsX=[a,d,c,f,g]
                    except:pass
                print('Run:',ChlpsX)
                if ChlpsX[0] == '=':print('E/EmpA')
                KeyRun(ChlpsX[0]);sls();aq=True
                # = empty
                if ChlpsX[1] != '=':KeyRun(ChlpsX[1]);sls();dq=True
                if ChlpsX[2] != '=':KeyRun(ChlpsX[2]);sls();cq=True
                if ChlpsX[3] != '=':KeyRun(ChlpsX[3]);sls();fq=True
                if ChlpsX[4] != '=':KeyRun(ChlpsX[4]);sls();gq=True
                try:
                    listZ=[1,2,3,4,5]
                    for x in listZ:
                        ...
                        try:
                            def az():
                                    if aq == True:keyboard.release(ChlpsX[0]);sls();aq == False
                            if f == 1:az()
                        except:pass
                        try:
                            def dz():
                                    if dq == True:keyboard.release(ChlpsX[1]);sls();dq == False
                            if f == 2:dz()
                        except:pass
                        try:
                            def cz():
                                    if cq == True:keyboard.release(ChlpsX[2]);sls();cq == False
                            if f == 3:cz()
                        except:pass
                        try:
                            def fz():
                                    if fq == True:keyboard.release(ChlpsX[3]);sls();fq == False
                            if f == 4:fz()
                        except:pass
                        try:
                            def gz():
                                    if gq == True:keyboard.release(ChlpsX[4]);sls();gq == False
                            if f == 5:gz()
                        except:pass
                        listZ.remove(f)
                except:pass
            ...
            def key32():
                ...
                print(synck)
                tryx=True
                Chlps=input('Give Keylist {1,5}/like a,d,f,h,b: ');Chlps=list(Chlps);Chlps=cleanC(Chlps)
                Chl=input('Give {1,2} hotkey for this macro: ');Chl=list(Chl);Chl=cleanC(Chl)
                ChlpsN = len(Chlps);ChlN = len(Chl)
                stnum=len(synck)
                if ChlN == 1:
                    if stnum == 1: tomkey (Chl[0],synck[0],'F1','F2','F3','F4','F5','F6',AutoKeyBS)
                    elif stnum == 2: tomkey (Chl[0],synck[0],synck[1],'F2','F3','F4','F5','F6',AutoKeyBS)
                    elif stnum == 3: tomkey (Chl[0],synck[0],synck[1],synck[2],'F3','F4','F5','F6',AutoKeyBS)
                    elif stnum == 4: tomkey (Chl[0],synck[0],synck[1],synck[2],synck[3],'F4','F5','F6',AutoKeyBS)
                    elif stnum == 5: tomkey (Chl[0],synck[0],synck[1],synck[2],synck[3],synck[4],'F5','F6',AutoKeyBS)
                    elif stnum == 6: tomkey (Chl[0],synck[0],synck[1],synck[2],synck[3],synck[4],synck[5],'F6',AutoKeyBS)
                    keyboard.add_hotkey(Chl[0],AutoKeyBS)
                elif ChlN == 2:
                    if stnum == 1: tomkey (Chl[0],Chl[1],synck[0],'F1','F2','F3','F4','F5',AutoKeyBS)
                    elif stnum == 2: tomkey (Chl[0],Chl[1],synck[0],synck[1],'F2','F3','F4','F5',AutoKeyBS)
                    elif stnum == 3: tomkey (Chl[0],Chl[1],synck[0],synck[1],synck[2],'F3','F4','F5',AutoKeyBS)
                ...
                elif ChlpsN == 3:Chlps=[Chlps[0],Chlps[1],Chlps[2],'=','=']
                elif ChlpsN == 4:Chlps=[Chlps[0],Chlps[1],Chlps[2],Chlps[3],'=']
                elif ChlpsN == 5:Chlps=[Chlps[0],Chlps[1],Chlps[2],Chlps[3],Chlps[4]]
                print(Chl)
            key32()
        keyboard.wait()
...
#################TermPy_conf#################
def TermPyS():
    sys('cls');color('TermPy 1.2',2)
    import code
    class MgiConsole(code.InteractiveConsole):
        import code, ctypes, subprocess, tempfile, random, string
